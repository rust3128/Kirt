...
BPosLogParser::BPosLogParser()
{
    m_entryStartRegex.setPattern("^(\\d{1,2}\\.\\d{2}\\.\\d{2}\\s\\d{2}:\\d{2}:\\d{2}\\.\\d{3})\\s+(.*)");
}

bool BPosLogParser::canParse(const QString& filePath)
{
    if (!QFileInfo(filePath).fileName().contains("BPos", Qt::CaseInsensitive)) {
        return false;
    }
...
// Функція: робить красивий XML з відступами
QString BPosLogParser::formatXml(const QString& xml) const
{
    QString formatted;
    int indentLevel = 0;
    bool inTag = false;
...
QList<LogEntry> BPosLogParser::parseFile(const QString& filePath, std::function<void(int)> progressCallback)
{
    ...
    while (!file.atEnd()) {
        ...
        if (match.hasMatch()) {
            if (hasCurrentEntry) {
                // === ШУКАЄМО ІНЛАЙН XML (xml=<...) ===
                int xmlStart = currentEntry.fullText.indexOf("xml=<");
                if (xmlStart != -1) {
                    xmlStart += 4; // Зсуваємо індекс на сам символ '<'
                    QString beforeXml = currentEntry.fullText.left(xmlStart);
                    QString xmlPart = currentEntry.fullText.mid(xmlStart);
                    // Додаємо переніс рядка перед XML, щоб він починався красиво
                    currentEntry.fullText = beforeXml + "\n" + formatXml(xmlPart);
                }
                // =====================================
