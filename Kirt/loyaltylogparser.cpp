...
LoyaltyLogParser::LoyaltyLogParser()
{
    // Стандартна дата: 26.02.24 00:03:04.292
    m_entryStartRegex.setPattern("^(\\d{1,2}\\.\\d{2}\\.\\d{2}\\s\\d{2}:\\d{2}:\\d{2}\\.\\d{3})\\s+(.*)");
}

bool LoyaltyLogParser::canParse(const QString& filePath)
{
    QString fileName = QFileInfo(filePath).fileName();

    // Шукаємо Loyalty, але відсікаємо MPosLoyalty
    if (!fileName.contains("Loyalty", Qt::CaseInsensitive) ||
        fileName.contains("mposloyalty", Qt::CaseInsensitive)) {
        return false;
    }
...
QList<LogEntry> LoyaltyLogParser::parseFile(const QString& filePath, std::function<void(int)> progressCallback)
{
